...
int main(void) {
    ...
    uint32_t *fb = mmap(0, framebuffer_info.height * framebuffer_info.width * framebuffer_info.bpp / 8, PROT_READ | PROT_WRITE,
				   MAP_SHARED, framebuffer_fd, 0);

    memewm_init(fb, framebuffer_info.width, framebuffer_info.height, framebuffer_info.pitch, font, 8, 16);
    memewm_window_create("Chalkboard", 30, 30, 800, 400);
    memewm_refresh();
	for (;;) {
		if (ioctl(mouse_fd, 0x1, &mouse_pack) == 0) {
            if (mouse_pack.x_mov == 0 || mouse_pack.y_mov == 0 && !(mouse_pack.flags & (1 << 0))) continue;
